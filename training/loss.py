...
class StyleGAN2Loss(Loss):
    ...
    def run_G(self, z, c, update_emas=False):
        cs = self.G.cond_xform(c, broadcast=True) # nothing to sync
        ws = self.G.mapping(z, cs[:, 0, :], update_emas=update_emas) # cs ignored unless concat mode
        if self.style_mixing_prob > 0:
            with torch.autograd.profiler.record_function('style_mixing'):
                cutoff = torch.empty([], dtype=torch.int64, device=ws.device).random_(1, ws.shape[1])
                cutoff = torch.where(torch.rand([], device=ws.device) < self.style_mixing_prob, cutoff, torch.full_like(cutoff, ws.shape[1]))
                ws[:, cutoff:] = self.G.mapping(torch.randn_like(z), cs[:, 0, :], update_emas=False)[:, cutoff:]
        img = self.G.synthesis(ws, cs, update_emas=update_emas)
        return img, ws
...
class ProjectedGANLoss(Loss):
    def __init__(self, device, G, D, blur_init_sigma=0, blur_fade_kimg=0, style_mixing_prob=0, augment_pipe=None, **kwargs):
        super().__init__()
        self.device = device
        self.G = G
        #self.G_ema = G_ema
        ...
        self.blur_fade_kimg = blur_fade_kimg
        self.style_mixing_prob = style_mixing_prob
        assert augment_pipe is None, 'ADA not yet supported'
