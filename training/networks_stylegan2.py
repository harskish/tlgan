...
class ToRGBLayer(torch.nn.Module):
    def __init__(self, in_channels, out_channels, w_dim, c_dim, cond_args, kernel_size=1, conv_clamp=None, channels_last=False):
        super().__init__()
        self.in_channels = in_channels
        self.out_channels = out_channels
        self.w_dim = w_dim
    ...
    def forward(self, x, w, c, fused_modconv=True):
        styles = self.scaled_styles(w, c) * self.weight_gain
...
class SynthesisBlock(torch.nn.Module):
    def __init__(self,
        ...
        out_channels,                           # Number of output channels.
        w_dim,                                  # Intermediate latent (W) dimensionality.
        c_dim,                                  # Conditioning dimensionality.
        ...
        **layer_kwargs,                         # Arguments for SynthesisLayer.
        ...
        if in_channels != 0:
            self.conv0 = SynthesisLayer(in_channels, out_channels, w_dim=w_dim, c_dim=c_dim, resolution=resolution, up=2,
                resample_filter=resample_filter, conv_clamp=conv_clamp, channels_last=self.channels_last, cond_args=cond_args, **layer_kwargs)
            self.num_conv += 1

        self.conv1 = SynthesisLayer(out_channels, out_channels, w_dim=w_dim, c_dim=c_dim, resolution=resolution,
            conv_clamp=conv_clamp, channels_last=self.channels_last, cond_args=cond_args, **layer_kwargs)
        self.num_conv += 1

        if is_last or architecture == 'skip':
            self.torgb = ToRGBLayer(out_channels, img_channels, w_dim=w_dim, c_dim=c_dim,
                conv_clamp=conv_clamp, channels_last=self.channels_last, cond_args=cond_args)
            self.num_torgb += 1
    ...
    def forward(self, x, img, ws, cs, force_fp32=False, fused_modconv=None, update_emas=False, **layer_kwargs):
        _ = update_emas # unused
        misc.assert_shape(ws, [None, self.num_conv + self.num_torgb, self.w_dim])
        misc.assert_shape(cs, [None, self.num_conv + self.num_torgb, self.c_dim])
        ...
        # Main layers.
        if self.in_channels == 0:
            x = self.conv1(x, next(w_iter), next(c_iter), fused_modconv=fused_modconv, **layer_kwargs)
        elif self.architecture == 'resnet':
            y = self.skip(x, gain=np.sqrt(0.5))
            x = self.conv0(x, next(w_iter), next(c_iter), fused_modconv=fused_modconv, **layer_kwargs)
            x = self.conv1(x, next(w_iter), next(c_iter), fused_modconv=fused_modconv, gain=np.sqrt(0.5), **layer_kwargs)
            x = y.add_(x)
        else:
            x = self.conv0(x, next(w_iter), next(c_iter), fused_modconv=fused_modconv, **layer_kwargs)
            x = self.conv1(x, next(w_iter), next(c_iter), fused_modconv=fused_modconv, **layer_kwargs)
        ...
        if self.is_last or self.architecture == 'skip':
            y = self.torgb(x, next(w_iter), next(c_iter), fused_modconv=fused_modconv)
...
class SynthesisNetwork(torch.nn.Module):
    def __init__(self,
        ...
        out_rect,                   # Valid part of output (masked datasets)
        ...
        **block_kwargs,             # Arguments for SynthesisBlock.
        ...
        for res in self.block_resolutions:
            ...
            block = SynthesisBlock(in_channels, out_channels, w_dim=w_dim, c_dim=self.c_dim, resolution=res,
                img_channels=img_channels, is_last=is_last, use_fp16=use_fp16, cond_args=cond_args, **block_kwargs)
    ...
    def forward(self, ws, cs, **block_kwargs):
        ...
        with torch.autograd.profiler.record_function('split_ws'):
            ...
            misc.assert_shape(cs, [None, self.num_ws, self.c_dim])
            ws = ws.to(torch.float32)
            w_idx = 0
            for res in self.block_resolutions:
                block = getattr(self, f'b{res}')
                block_ws.append(ws.narrow(1, w_idx, block.num_conv + block.num_torgb))
                block_cs.append(cs.narrow(1, w_idx, block.num_conv + block.num_torgb))
                w_idx += block.num_conv

        x = img = None
        for res, cur_ws, cur_cs in zip(self.block_resolutions, block_ws, block_cs):
            block = getattr(self, f'b{res}')
            x, img = block(x, img, cur_ws, cur_cs, **block_kwargs)

        # Mask output based on content rect
        # Improves convergence on masked datasets
        out = -1 * torch.ones_like(img) # range [-1, 1] maps to [0, 255]
        x1,y1,x2,y2 = self.out_rect
...
class Generator(torch.nn.Module):
    def __init__(self,
        ...
        **synthesis_kwargs,         # Arguments for SynthesisNetwork.
        ...
        self.img_resolution = img_resolution
        self.img_channels = img_channels
        self.synthesis = SynthesisNetwork(w_dim=w_dim, cond_args=cond_args, \
            img_resolution=img_resolution, img_channels=img_channels, **synthesis_kwargs)
        self.num_ws = self.synthesis.num_ws
        self.cond_args = cond_args
        c_dims_M = {'concat': c_dim, 'f_concat': cond_args.dims}.get(cond_args.type, 0)
        self.mapping = MappingNetwork(z_dim=z_dim, c_dim=c_dims_M, w_dim=w_dim, num_ws=self.num_ws, **mapping_kwargs)
        self.cond_xform = ConditioningTransform(cond_args=cond_args, num_ws=self.num_ws)
...
class Discriminator(torch.nn.Module):
    def __init__(self,
        ...
        super().__init__()
        _ = c_dim  # using cond_args.dims instead
        self.c_dim = cond_args.dims
        self.img_resolution = img_resolution
        self.img_resolution_log2 = int(np.log2(img_resolution))
        self.img_channels = img_channels
        ...
        self.cond_xform = ConditioningTransform(cond_args=cond_args, add_noise=True)
